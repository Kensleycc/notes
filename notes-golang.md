...
- [协程调度](#协程调度)
  ...
  - [GMP 调度模型](#gmp-调度模型)
    ...
    - [调度时机](#调度时机)
    - [系统调用](#系统调用)
      - [当 Goroutine 阻塞时，P 是否阻塞？](#当-goroutine-阻塞时p-是否阻塞)
- [垃圾回收](#垃圾回收)
  - [标记清扫](#标记清扫)
    - [缺点](#缺点)
  - [三色标记法](#三色标记法)
...
# channel
## 有无缓冲的区别
- 无缓冲时为同步channel：`无缓冲 channel，发送者会阻塞直到接收者接收了发送的值。`
___
...
- 加锁时，如果 Locked = 1（已加锁），满足以下 **自旋条件** 情况下，协程会自旋，而非马上阻塞：
...
#### 当 Goroutine 阻塞时，P 是否阻塞？
两种情况：
1. **同步调用**（file IO）—— GM 阻塞
2. **异步调用**（Golang Net IO）—— 只阻塞 G
   - 网络IO不阻塞线程是因为Golang使用IO多路复用实现网络库（linux-epoll、macos——kqueue、windows——iocp）
___
# 垃圾回收
## 标记清扫
- 全程开启 STW
...
___
## 屏障技术
**读写屏障技术**，通过 hook 拦截堆、栈对象的读写操作，触发对应的屏障代码。

读屏障通常性能影响较大，因此只考虑写屏障的实现。
要解决三色标记法 STW 的问题，需要满足强弱三色不变式其中之一：
### 三色不变式
- 强三色不变式：黑色不能引用白色。
- 弱三色不变式：被解引用的白色，需要存在灰色上游。

### 插入写屏障
- 栈不启动
- 非黑对象被引用时，标记为灰色
- 由于栈不启动，会遗漏栈上黑色对象新引用的白色对象。因此扫描完后，需要 STW，rescan 栈空间，避免错删
- 删除所有白色对象
- **缺点：** 仍需要 STW rescan

### 删除写屏障
- 栈不启动
- GC开始时，扫描栈 **STW** 生成一个栈快照，标记所有对象为黑色。
- 解引用的对象标记为灰色
- **缺点：**回收精度低，会产生应删除的对象活到下一轮 GC。
...
- 相比较于删除屏障，在扫描栈的时候，无需STW，而是一个栈一个栈的 block。
