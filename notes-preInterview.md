...
- 核心问题
1. 处理超卖
   - 悲观锁：数据库或者加分布式锁中间件。**缺点**是并发性能较差，大量阻塞请求。
   - 乐观锁：提交时加锁。**缺点**是库存不足时大量请求回滚，性能会受影响甚至更差。
   - 消息队列：消息的有序消费。需要维护消息队列的可用性。
2. 大量无效、非法的请求
   - 限流：
     - `前端限流`如验证码、重复点击限制；
     - `后端限流`如限流器：**固定限流**（n*k请求后失败，适用秒杀）；**令牌桶**（削峰填谷蓄洪，适用于持续高并发）
___
## 给定一个敏感词库，判断大文件中是否存在敏感词
- 朴素：大文件分块处理，每个块全量文本遍历敏感词库，每个词做KMP匹配要做分块首尾连接特殊处理，防止敏感词被截断的情况。
  - 复杂度：O(K(m+n))：每次KMP匹配为 O(m+n)，m是敏感词长度，n是文本的长度，一共K个敏感词。
  - 适合词库小，
- 字典树：将敏感词库构建字典树，遍历全量文本（大文件则分块遍历）
- AC自动机：字典树的基础上，加上失配指针（类似KMP部分文本匹配失败的回退）
___
## 大词库中统计频数Top10的单词
- 思路：大词库分块处理，每个分块统计词频，最后汇总
  - HASH统计：需要额外使用hash表外存，**空间消耗**大
    - **分治：** 每个分块维护一个hash表统计分块词频，然后hash表写入外存，循环递归下一个文件。
    - **归并：** 维护一个 `TOP K 大顶堆`，遍历每一个hash外存，循环入堆
  - 排序归并：需要分块排序，**时间消耗**大
    - **分治**：每个分块进行排序
    - **归并**：维护一个 `TOP K 大顶堆`，遍历每一个分块，循环入堆
___
## 
